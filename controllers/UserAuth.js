      ...
      bcrypt
        ...
        .then((result) => {
          if (result) {
            ...
            const token = jwt.sign({ userId: user._id }, "SOMETHINGSECRET", {
              expiresIn: "1h",
            });
  ...
  User.findOne({ _id: req.userId })
    .then((user) => {
      if (user) {
        res
          .status(200)
          .json({ tasks: user.userTasks, userName: user.userName });
      }
    })
    .catch(() => {});
};
  ...
  User.findOne({ _id: userId })
    .then((user) => {
      if (user) {
        ...
        user.userTasks = user.userTasks.filter(
  ...
  User.findOne({ _id: userId })
    .then((user) => {
      if (user) {
        ...
        user.userTasks = user.userTasks.map((task) => {
          if (task._id == taskId) {
            task.status = true;
          }
          return task;
